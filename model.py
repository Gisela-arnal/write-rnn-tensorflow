...
class Model():
    def __init__(self, args, infer=False):
        ...
        flat_target_data = tf.reshape(self.target_data, [-1, 75])
        ...
        def get_mixture_coef(output):
            ...
            z = output
            z_eos = z[:, 0:1]
            z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr = tf.split(1, 6, value=z[:, 1:])
    ...
    def sample(self, sess, num=1200):
        ...
        strokes = np.zeros((num, 3*25), dtype=np.float32)
        mixture_params = []

        for i in range(num):
            feed = {self.input_data: prev_x, self.state_in: prev_state}

            [o_pi, o_mu1, o_mu2, o_sigma1, o_sigma2, o_corr, o_eos, next_state] = sess.run(
                [self.pi, self.mu1, self.mu2, self.sigma1, self.sigma2, self.corr, self.eos, self.state_out], feed)

            idx = get_pi_idx(random.random(), o_pi[0])

            eos = 1 if random.random() < o_eos[0][0] else 0

            next_x1, next_x2 = sample_gaussian_2d(o_mu1[0][idx], o_mu2[0][idx], o_sigma1[0][idx], o_sigma2[0][idx],
                                                  o_corr[0][idx])
